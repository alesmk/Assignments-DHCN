...
#define TIMERSTART(timename) auto timename = std::chrono::system_clock::now();
#define TIMERSTOP(timename)  {auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - timename).count(); std::cout << elapsed << " usecs" << std::endl; }
